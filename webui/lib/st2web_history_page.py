...
class St2webHistoryPage(St2webCommon):
    ...
    def get_output_of_last_execution(self, action_name):
        return self.get_last_execution_value(action_name, 'output', st2web_locators.HISTORY_EXECUTION_RESULT_STDOUT)

    def get_status_of_last_execution(self, action_name):
        return self.get_last_execution_value(action_name, 'status', st2web_locators.HISTORY_EXECUTION_RESULT_STATUS)

    def get_trigger_payload_of_last_execution(self, action_name):
        return self.get_last_execution_value(action_name, 'trigger payload', st2web_locators.HISTORY_EXECUTION_RESULT_TRIGGER_PAYLOAD)
    ...
    def get_last_execution_value(self, action_name, field_name, field_xpath):
        ...
        field_panel = history_page.wait(ImplementationFactory().get_wait_for_element_to_be_present(field_xpath))
